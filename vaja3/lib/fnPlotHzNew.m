function fnPlotHzNew(n,p,zrange,zstep,wstep,b0)
...
%fig=figure;
h=surf(zreal,zimag,Az); colormap hsv
caxis([0 10])
hold on, 
plot3(zwreal,zwimag,Aw,'m','LineWidth',3),
axis([min(zwreal) max(zwreal) min(zwimag) max(zwimag) 0 3])
