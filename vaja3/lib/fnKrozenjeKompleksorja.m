function fnKrozenjeKompleksorja(A,phi0,om,n)

fig=figure; set(fig,'Position',[250,150,350-40,2*350])
...
for i=1:N
    ...
    fnPlotSignal(A,om*n(1:i-1)+phi0,n(1:i-1),n(1),n(end),[0.5 0.5 1]), hold on
    fnPlotSignal(A,om*n(i)+phi0,n(i),n(1),n(end),[0 0 0.5])
end
...
function fnPlotSignal(A,phi,ntemp,nmin,nmax,color)
    plot(A*cos(phi),ntemp,'.','MarkerSize',12,'Color',color)
    axis([-A, A,nmin, nmax])
    title('Realen signal'),xlabel('{\itx}[{\itn}]'),ylabel('\itn')
end
